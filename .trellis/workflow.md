...
- 2.2b Structure audit `[recommended · repeatable]`
...
Spawn the structure-audit sub-agent alongside (after) the check sub-agent:

- **Agent type**: `trellis-structure-audit`
- **Task description**: Audit code and project structure of files this task reads, modifies, or creates; report at most one structural finding
- **Dispatch prompt guard**: The prompt MUST start with `Active task: <task path>`, then tell the spawned agent it is already the `trellis-structure-audit` sub-agent and must audit/report directly, not spawn another sub-agent.

When to dispatch:
- After each implement chunk that adds files, moves files, creates modules, or clearly touches architecture (2.2b follows the same repeatable cadence as 2.2).
- Always as part of the final 2.2 full-scope pass, before Phase 3.4 commit.
- Skip when the chunk is a small in-place edit with no structural surface (no new/moved files, no module changes).

The audit agent's job (see `.trellis/agents/structure-audit.md` / `.claude/agents/trellis-structure-audit.md`):
- Structural code smells (function size, complexity, nesting, god classes, duplication, coupling) with conservative, language-adjusted thresholds
- Project organization (file placement, junk-drawer directories, scattered domains, dead files, convention violations)
- At most ONE finding per activation, with path, evidence, impact, and minimal fix direction; silence when nothing is above threshold

The main session decides what to act on: findings are advisory input to 2.3 rollback / 3.3 spec update, not auto-fixes. The audit agent never edits code or commits.

[/Claude Code, Cursor, OpenCode, codex-sub-agent, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi, Oh My Pi, ZCode, Snow, Reasonix, Trae, Grok, Kimi Code]

[codex-inline, Kilo, Antigravity, Devin, DeepSeek Harness]

Inline platforms have no sub-agent dispatch: load the `trellis-structure-audit` agent definition (`.trellis/agents/structure-audit.md`) as a checklist and apply it directly to the current diff — same thresholds, same one-finding cap, same silence-when-below-threshold rule.
